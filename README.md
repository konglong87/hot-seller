...
git clone git@github.com:konglong87/hot-seller.git .claude/skills/hot-seller-pack
...
安装后，在 Claude Code 中输入 `/hot-seller-pack` 即可调用，或当你的提问涉及销售、定价、带货等触发词时自动激活。
...
当涉及销售策略、定价、带货等问题时，参考 `.claude/skills/hot-seller-pack/SKILL.md` 中的三位大师视角。
```
...
# 或手动安装
git clone git@github.com:konglong87/hot-seller.git ~/.openclaw/skills/hot-seller-pack
```

安装后，在 OpenClaw 中输入 `/hot-seller-pack` 即可调用，或当提问涉及销售触发词时自动激活。
